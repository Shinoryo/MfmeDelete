...
function buildMonthRange(options) {
  ...
  const newer = compareYearMonth(left, right) >= 0 ? left : right;
  const older = compareYearMonth(left, right) >= 0 ? right : left;
  const months = [];
...
async function deleteRows(page, options) {
  ...
  const runWorker = async () => {
    while (true) {
      ...
      try {
        const deleted = await deleteSingleRow(page, options, sequence, target);
        if (!deleted) {
          return;
        }
